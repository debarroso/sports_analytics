...
class ProFootballRefPlayersParser(BaseParser):
    ...
    def parse(self):
        for player_file in self.files:
            ...
            self.logger.info(
                f"Processing file {pathlib.Path(player_file).parts[-1]} took {time.perf_counter() - start_time}"
            )
    ...
    def extract_player_details(self, file_name=""):
        if file_name == "":
            self.logger.error(
                f"Function extract_player_details called without a file_name"
            )
        ...
        height_weight_search = re.search(
            r"(\d+-\d+)\s*,\s*(\d+)lb\s*\((\d+cm),\s*(\d+kg)\)", player_string
        )
        if height_weight_search:
            player_data["height"] = height_weight_search.group(1)
            player_data["weight_lb"] = int(
                height_weight_search.group(2).replace("lb", "")
            )
            player_data["height_cm"] = int(
                height_weight_search.group(3).replace("cm", "")
            )
            player_data["weight_kg"] = int(
                height_weight_search.group(4).replace("kg", "")
            )

        dob_search = re.search(r"Born:\s*(.*)", player_string)
        if dob_search:
            date_string = (
                dob_search.group(1).replace("\xa0", " ").split("(Age")[0].strip()
            )
            player_data["date_of_birth"] = datetime.strptime(
                date_string, "%B %d, %Y"
            ).strftime("%Y-%m-%d")
        ...
        draft_search = re.search(
            r"Draft\s*:\s*.*\((\d+)(?:st|nd|rd|th)\s*overall\)", player_string
        )
        if draft_search:
        ...
        try:
            ...
            if str(e) == "No tables found matching pattern 'Combine Measurements'":
                self.logger.info(
                    f"No combine measurements for {pathlib.Path(file_name).parts[-1]}"
                )
            else:
                raise e
